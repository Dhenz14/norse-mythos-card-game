...
export function calculateSidePots(
  ...
  if (excess > 0) {
    ...
    console.log(`[SidePot] Uncalled bet of ${excess} for ${excessPlayer.playerName} (will be returned)`);
  }
...
export function awardPots(
  ...
  console.log(`[SidePot] ${winnerId} wins main pot of ${potState.mainPot}`);
  ...
  for (const sidePot of potState.sidePots) {
    if (sidePot.eligiblePlayers.includes(winnerId)) {
      ...
      console.log(`[SidePot] ${winnerId} wins side pot of ${sidePot.amount}`);
      ...
      console.log(`[SidePot] ${returnTo} has uncalled bet of ${sidePot.amount} returned`);
    }
  }
