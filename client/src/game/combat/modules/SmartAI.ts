...
export function getSmartAIAction(
  ...
  console.log(`[SmartAI] Phase: ${combatState.phase}, Hand strength: ${(handStrength * 100).toFixed(1)}%, Pot odds: ${(potOdds * 100).toFixed(1)}%`);
  console.log(`[SmartAI] Position: ${actorPosition}, firstBettingRound: ${isFirstBettingRound}, hasBetToCall: ${hasBetToCall}, toCall: ${toCall}`);
  console.log(`[SmartAI] availableHP: ${availableHP}, currentBet: ${combatState.currentBet}, actorHpCommitted: ${actor.hpCommitted}`);
  
  if (availableHP <= 0) {
    console.log(`[SmartAI] DECISION: BRACE (no HP available)`);
    return {
      action: CombatAction.BRACE,
      betAmount: 0,
      reasoning: 'No HP available, must fold'
    };
  }
