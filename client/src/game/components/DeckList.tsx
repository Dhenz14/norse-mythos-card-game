...
const DeckList: React.FC<DeckListProps> = ({
  ...
  const cardsByMana = useMemo(() => {
    ...
    deckCards.forEach(item => {
      const manaCost = item.card.manaCost >= 7 ? 7 : item.card.manaCost; // Group 7+ together
            ...
            {cards.map(({ card, count }) => (
              ...
              >
                <div className="flex items-center">
                  ...
                  <div className="mana-cost w-6 h-6 bg-blue-500 rounded-full flex items-center justify-center text-white font-bold shadow-md mr-2">
                    {card.manaCost}
                  </div>
