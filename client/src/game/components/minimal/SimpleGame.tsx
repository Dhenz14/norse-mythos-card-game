...
const Card = ({ 
  ...
  const health = isBattlefield ? card.currentHealth : card.card.health;
...
export const SimpleGame = () => {
  ...
  const playCardSimple = (card: CardInstance) => {
    ...
    if (card.card.manaCost > players.player.mana.current) {
            ...
            <div className="flex flex-wrap gap-4 justify-center">
              {players.player.hand.length === 0 ? (
                ...
                players.player.hand.map(card => {
                  const canPlay = currentTurn === 'player' && card.card.manaCost <= players.player.mana.current;
