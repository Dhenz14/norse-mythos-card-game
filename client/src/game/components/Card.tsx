...
// Extend the CardInstance type to include isGolden property
...
export const Card = React.forwardRef<HTMLDivElement, CardProps>(({
  ...
  if ('card' in card && !isInHand) {
    ...
    console.log(`Card instance: ${cardData.name}, Attack: ${cardData.attack}, Type: ${cardData.type}`);
  }
  ...
  const raceText = cardData.race || cardData.tribe || '';
    ...
    >
      ...
      {cardData.attack !== undefined && (
        <div className={`card-corner-ornament card-corner-ornament-bl card-stat-gem ${cardData.rarity || 'common'}`} style={{ zIndex: 1100 }}>
          <span className="stat-value">{cardData.attack}</span>
        </div>
      )}
      ...
      {isInHand && (
        <div className="absolute -bottom-10 left-0 right-0 flex justify-center">
          <button
            ...
            disabled={!isPlayable}
          >
            {isPlayable ? 'PLAY' : `NEED ${cardData.manaCost - ((card as any).currentMana || 0)} MANA`}
          </button>
        </div>
      )}
