 ...
 * Example card: Hakkar, the Soulflayer (ID: 20602)
 */
...
export function executeShuffleCardShuffleCard(
  ...
  console.log(`Executing shuffle_card deathrattle for ${sourceCard.card.name}`);
  
  // Check for required property: condition
  if (effect.condition === undefined) {
    console.warn(`ShuffleCard effect missing condition property`);
  ...
  // TODO: Implement the shuffle_card deathrattle effect
  ...
  newState.gameLog.push({
    ...
    type: 'deathrattle',
    text: `${sourceCard.card.name} triggered shuffle_card deathrattle`,
    timestamp: Date.now(),
