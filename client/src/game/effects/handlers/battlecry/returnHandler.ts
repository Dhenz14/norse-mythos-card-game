...
export function executeReturnReturn(
  ...
  const currentPlayerId = newState.activePlayerId || 'player';
  const currentPlayer = newState.players[currentPlayerId];
  ...
  const handMinion: CardInstance = {
    ...
    currentHealth: targetMinion.card.health || 1, // Reset health to full
    hasDivineShield: !!targetMinion.card.keywords?.includes('divine_shield'), // Reset divine shield
    isPoisonous: !!targetMinion.card.keywords?.includes('poisonous'), // Reset poisonous
