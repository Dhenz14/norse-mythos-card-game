...
function executeBuffFromGraveyardCountBuffFromGraveyardCount(
  ...
  if (totalBuff <= 0) {
    return { success: true, info: `No buff applied - empty graveyard` };
  }
  ...
  try {
    ...
    if (sourceCard) {
      sourceCard.attack = (sourceCard.attack || 0) + totalBuff;
      sourceCard.health = (sourceCard.health || 0) + totalBuff;
      sourceCard.maxHealth = (sourceCard.maxHealth || 0) + totalBuff;
    }
    
    return { 
      success: true, 
      info: `Buffed by +${totalBuff}/+${totalBuff} based on ${minionCount} minions in graveyard`
    };
  } catch (error) {
