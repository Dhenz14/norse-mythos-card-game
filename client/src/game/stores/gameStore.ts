...
import { createCardInstance, findCardInstance } from '../utils/cardUtils';
import { 
  initializeGame, 
  playCard, 
  endTurn, 
    ...
    try {
      ...
      try {
        ...
        if (cardInstance.card.type === 'spell' && 
          ...
          // Auto-attack ONLY if the minion has Charge keyword
          // Charge minions can attack immediately, but we delay slightly for visual feedback
          // Rush minions can attack minions immediately but NOT auto-attack (player chooses target)
          let finalState = newState;
          const hasCharge = Array.isArray(cardInstance.card.keywords) && cardInstance.card.keywords.includes('charge');
          
          if (cardInstance.card.type === 'minion' && cardInstance.card.attack && cardInstance.card.attack > 0 && hasCharge) {
            debug.log(`[AutoAttack] ${cardInstance.card.name} has CHARGE - will auto-attack after appearing on battlefield`);
            
            // First update state to show the card on battlefield
            set({ 
              gameState: newState,
              selectedCard: null
            });
            
            // Then delay the auto-attack so the card summoning animation completes first
            // Increased from 600ms to 1200ms to ensure card is fully visible and animation settles
            setTimeout(() => {
              const currentState = get().gameState;
              
              // Safety check: verify the minion still exists on battlefield before attacking
              const minionStillExists = currentState.players.player.battlefield.some(
                (c: any) => c.instanceId === cardInstance.instanceId
              );
              
              if (!minionStillExists) {
                debug.log(`[AutoAttack] ${cardInstance.card.name} no longer on battlefield - skipping auto-attack`);
                return;
              }
              
              const attackState = autoAttackOnPlace(currentState, cardInstance.instanceId, 'player');
              set({ gameState: attackState });
            }, 1200); // 1200ms delay to ensure summoning animation completes
            
            return; // Exit early since we're handling state update in timeout
          }
      ...
      setTimeout(() => {
        ...
        if (newState !== freshState) {
          ...
          if (freshAttackerCard) {
            const damage = ('attack' in freshAttackerCard.card ? freshAttackerCard.card.attack : 0) || 0;
            const targetMinion = freshState.players.opponent.battlefield.find(c => c.instanceId === defenderId);
            const counterDamage = (targetMinion?.card && 'attack' in targetMinion.card ? targetMinion.card.attack : 0) || 0;
          ...
          if (freshAttackerCard) {
            ...
            logActivity('attack', 'player', `${freshAttackerCard.card.name} attacked ${targetName}`, {
              ...
              value: ('attack' in freshAttackerCard.card ? freshAttackerCard.card.attack : 0)
            });
          }
