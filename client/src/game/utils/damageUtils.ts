...
export function dealDamageToHero(
  ...
  if (remainingDamage > 0) {
    targetPlayer.heroHealth = Math.max(0, targetPlayer.heroHealth - remainingDamage);
  }
  ...
  newState.gameLog.push({
    ...
    source: sourceCardId?.toString(),
    target: 'hero-' + targetPlayerId,
  });
  
  // Check for game over
  if (targetPlayer.heroHealth <= 0) {
...
export function dealDamageToMinion(
  ...
  const targetIndex = battlefield.findIndex(card => card.instanceId === targetInstanceId);
  ...
  if (targetCard.currentHealth !== undefined) {
    ...
    const oldHealth = targetCard.currentHealth || targetCard.card.health || 0;
  ...
  newState.gameLog.push({
    id: uuidv4(),
    type: 'damage',
    text: `${sourcePlayerId ? 'Player ' + sourcePlayerId : 'Something'} dealt ${amount} damage to ${targetCard.card.name}.`,
    timestamp: Date.now(),
    turn: newState.turnNumber,
    source: sourceCardId?.toString(),
    target: targetInstanceId,
    message: `${amount} damage was dealt to ${targetCard.card.name} (${targetCard.currentHealth} HP remaining)`
  });
  ...
  playerMinions.forEach((minion, index) => {
    ...
    if (minionCopy.hasDivineShield) {
      ...
      newState.gameLog.push({
        ...
        turn: newState.turnNumber,
        timestamp: Date.now(),
        text: `${amount} damage was dealt to ${minionCopy.card.name}`,
        targetId: minionCopy.instanceId,
        targetPlayerId: defendingPlayerId,
        message: `${amount} damage was dealt to ${minionCopy.card.name} (${oldHealth} -> ${minionCopy.currentHealth})`
      });
    }
...
export function dealDamageToAllMinions(
  ...
  if (playerMinions && playerMinions.length > 0) {
    ...
    // Apply all the changes to the actual state
    playerMinionsToUpdate.forEach(({ minion, index }) => {
      // Update the minion in the actual battlefield
      playerMinions[index] = minion;
    });
  }
  ...
  if (opponentMinions && opponentMinions.length > 0) {
    ...
    // First, check Divine Shield and record damage to apply
    opponentMinions.forEach((minion, index) => {
      // Create a copy for modification
      const minionCopy = { ...minion };
...
export function healHero(
  ...
  const currentHealth = targetPlayer.heroHealth;
  ...
  // Apply healing
  targetPlayer.heroHealth += actualHealing;
...
export function healMinion(
  ...
  if (targetCard.currentHealth === undefined || 
      targetCard.card.health === undefined ||
      targetCard.currentHealth >= targetCard.card.health) {
    return newState; // No healing needed
  }
  
  // Calculate actual healing
  const maxHealth = targetCard.card.health;
  ...
  newState.gameLog.push({
    id: uuidv4(),
    type: 'heal',
    text: `${targetCard.card.name} was healed for ${actualHealing}.`,
    timestamp: Date.now(),
    turn: newState.turnNumber,
    source: sourceCardId?.toString(),
    target: targetInstanceId,
...
export function getValidTargets(
  ...
  if (targetType === 'damaged_minion') {
    return validTargets.filter(target => {
      ...
      let minion = playerField.find(m => m.instanceId === target.targetId);
      if (!minion) {
        minion = opponentField.find(m => m.instanceId === target.targetId);
      }
