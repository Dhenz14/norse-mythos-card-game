...
import { processAfterHeroAttackEffects } from './afterAttackUtils';
...
export function equipWeapon(
  ...
  const weapon: CardInstance = {
    ...weaponCard,
    isPlayed: true,
    currentDurability: weaponCard.card.durability || 1, // Set initial durability
    attacksPerformed: 0,
  ...
  const logEvent: GameLogEvent = {
    ...
    timestamp: Date.now(),
    player: playerType,
    text: `${playerType} equipped ${weapon.card.name} (${weapon.card.attack}/${weapon.currentDurability})`,
...
export function attackWithWeapon(
  ...
  const attackDamage = weapon.card.attack || 0;
  ...
  if (!targetId || targetType === 'hero') {
    ...
    if (targetMinion.hasDivineShield) {
      ...
      const currentHealth = targetMinion.currentHealth || targetMinion.card.health || 0;
    ...
    newState.players[attackingPlayerType].health -= targetMinion.card.attack || 0;
...
export function canPlayerAttackWithWeapon(state: GameState, playerType: 'player' | 'opponent'): boolean {
  ...
  if ((player.weapon.card.attack || 0) <= 0) {
    return false;
  }
...
export function isValidWeaponTarget(
  ...
  if (hasTaunt && targetType === 'minion' && targetId) {
    ...
    if (targetMinion && !targetMinion.card.keywords.includes('taunt')) {
      return false;
    }
  }
...
export function buffWeapon(
  ...
  // Apply buffs
  const originalAttack = weapon.card.attack || 0;
  const buffedAttack = originalAttack + attackBuff;
  
  const originalDurability = weapon.currentDurability || weapon.card.durability || 0;
