...
import { GameState, CardData, OverloadEffect } from '../types';

/**
 * Check if a card has overload
 * @param card Card data to check
 * @returns True if the card has overload effect
 */
export function hasOverload(card: CardData): boolean {
  return card.keywords.includes('overload') && card.overload !== undefined;
}
