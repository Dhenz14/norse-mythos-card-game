...
export const handleDiscover = (
  ...
  const discoveryState: DiscoveryState = {
    ...
    filters: {
      type: discoveryType === 'any' ? 'any' : discoveryType,
...
export const handleTradeable = (
  ...
  const tradeCost = cardInstance.card.tradeableInfo?.tradeCost || 1;
...
export const handleInspireEffects = (
  ...
  const inspireMinions = battlefield.filter(
    minion => minion.card.keywords.includes('inspire')
  );
  
  // Process each inspire effect
  for (const minion of inspireMinions) {
    const inspireEffect = minion.card.inspireEffect;
            ...
            if (m.instanceId !== minion.instanceId) { // Don't buff the source minion
              return {
                ...
                currentHealth: m.currentHealth 
                  ...
                  : m.card.health ? m.card.health + (inspireEffect.buffHealth || 0) : undefined
              };
            }
            return m;
          });
...
export const isCardUsableByClass = (
  ...
  if (card.dualClassInfo && card.dualClassInfo.classes.includes(heroClass)) {
    return true;
  }
...
export const handleChooseOneSelection = (
  ...
  if (!cardInstance.card.chooseOneOptions || 
      optionIndex < 0 || 
      optionIndex >= cardInstance.card.chooseOneOptions.length) {
    return gameState;
  }
...
export const handleSecretTrigger = (
  ...
  const matchingSecrets = playerSecrets.filter(secret => 
    secret.card.secretEffect && 
    secret.card.secretEffect.triggerType === triggerType &&
    !secret.isRevealed
  );
  ...
  for (const secret of matchingSecrets) {
    if (!secret.card.secretEffect) continue;
    
    const secretEffect = secret.card.secretEffect.effect;
          ...
          if (targetIndex !== -1) {
            ...
            if (targetMinion.currentHealth) {
              ...
              if (targetMinion.currentHealth <= 0) {
                ...
                newGameState.players[triggeringPlayer].graveyard.push(targetMinion.card);
              }
            }
          }
        }
      ...
      case 'transform':
        ...
        if (secretEffect.targetType === 'none') {
          ...
          if (targetIndex !== -1) {
            ...
            minion.originalManaCost = minion.card.manaCost;
            minion.card = {
              ...minion.card,
              manaCost: minion.card.manaCost + 2
            };
    ...
    newGameState.players[secretOwner].secrets = playerSecrets.filter(s => !s.isRevealed);
...
export const canBeCorrupted = (
  ...
  if (!corruptibleCard.keywords.includes('corrupt') || 
      !corruptibleCard.corruptState || 
      !corruptibleCard.corruptState.isCorruptible ||
      corruptibleCard.corruptState.isCorrupted) {
    return false;
  }
  
  // Check if played card costs more
  return playedCard.manaCost > corruptibleCard.manaCost;
};
...
export const handleCorruption = (
  ...
  for (let i = 0; i < playerHand.length; i++) {
    ...
    if (canBeCorrupted(card, playedCard)) {
      // Corrupt the card
      if (card.corruptState && card.corruptState.corruptedVersion) {
...
export const filterDiscoverOptions = (
  ...
  criteria: {
    ...
    keywords?: CardKeyword | CardKeyword[];
    excludeKeywords?: CardKeyword | CardKeyword[];
  ...
  return options.filter(card => {
    ...
    if (criteria.costRange) {
      ...
      if (card.manaCost < min || card.manaCost > max) {
        return false;
      }
    }
    
    // Filter by exact cost
    if (criteria.exactCost !== undefined && card.manaCost !== criteria.exactCost) {
      return false;
    }
