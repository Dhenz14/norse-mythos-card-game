...
import { GameState, CardData, CardInstance, GameLogEventType } from '../types';
...
import { getRandomInt } from '@/game/utils/randomUtils';
...
export function buffCThun(
  ...
  // Update any C'Thun card in the player's hand, deck, or battlefield
  const updateCardStats = (card: CardInstance): CardInstance => {
    if (card.card.id === 60001) { // C'Thun's ID
  ...
  newState.players[playerType].hand = newState.players[playerType].hand.map(updateCardStats);
  newState.players[playerType].deck = newState.players[playerType].deck.map(updateCardStats);
  newState.players[playerType].battlefield = newState.players[playerType].battlefield.map(updateCardStats);
...
export function isCThunPowered(
  ...
  if (!state.players[playerType].cthunData) {
    return false;
  }
  
  // Check if C'Thun has at least 10 attack
  return state.players[playerType].cthunData.currentAttack >= 10;
}
...
export function executeCThunBattlecry(
  ...
  const damageAmount = cthun.card.attack;
...
export function executeNZothBattlecry(
  ...
  const deathrattleMinions = graveyard.filter(
    (card: CardInstance) => card.card.type === 'minion' && 
  ...
  for (const minion of minionsToResurrect) {
    ...
    const newMinion: CardInstance = {
      ...
      attacksPerformed: 0,
      currentHealth: minion.card.health
    };
  ...
  for (let i = 0; i < spellsCast; i++) {
    ...
    if (!yoggStillExists) {
      newState.gameLog.push(
        createGameLogEvent(
      newState,
      'yogg_saron_stopped' as GameLogEventType,
      playerType,
      `Yogg-Saron was destroyed, stopping spell casting.`,
      { cardId: 60102 }
    )
      );
      break;
    ...
    newState.gameLog.push(
      createGameLogEvent(
        newState,
        'yogg_saron_cast' as GameLogEventType,
        playerType,
        `Yogg-Saron casts ${randomSpell.name}.`,
...
export function executeYShaarjEffect(
  ...
  if (deck.length === 0) {
    newState.gameLog.push(
      createGameLogEvent(
      newState,
      'yshaarj_effect_failed' as GameLogEventType,
      playerType,
      `Y`,
      { cardId: 60103 }
    )
  ...
  // Filter for minions in deck
  const minionsInDeck = deck.filter((card: CardInstance) => card.card.type === 'minion');
  ...
  const minionToSummon = minionsInDeck[randomIndex];
  
  // Remove from deck
  newState.players[playerType].deck = deck.filter(
    (card: CardInstance) => card.instanceId !== minionToSummon.instanceId
  );
  
  // Add to battlefield
  newState.players[playerType].battlefield.push({
    ...minionToSummon,
    isSummoningSick: true,
    attacksPerformed: 0
  });
  ...
  newState.gameLog.push(
    createGameLogEvent({
      ...
      player: playerType,
      text: `Y'Shaarj summoned ${minionToSummon.card.name} from your deck.`,
      cardId: minionToSummon.card.id.toString()
    })
  );
