...
export function initializeDormantEffect(card: CardInstance): CardInstance {
  ...
  if (card.card.keywords.includes('dormant')) {
    return {
      ...card,
      isDormant: true,
...
function processDormantForPlayer(
  ...
  for (let i = 0; i < battlefield.length; i++) {
    ...
    if (minion.card.customAwakeningCondition) {
      continue;
    }
        ...
        for (const card of hand) {
          if (card.card.type === 'minion') {
            ...
            if (awakenEffect.buffAttack) {
              card.card.attack = (card.card.attack || 0) + awakenEffect.buffAttack;
            }
            
            // Apply health buff
            if (awakenEffect.buffHealth) {
              card.card.health = (card.card.health || 0) + awakenEffect.buffHealth;
            }
          }
        }
...
/**
 * Check if a card can attack based on its dormant state
 * @param card The card to check
 * @returns True if the card can attack
 */
