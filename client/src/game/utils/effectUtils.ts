  ...
  for (let i = 0; i < enemyBattlefield.length; i++) {
    ...
    updatedState.gameLog = [
      ...(updatedState.gameLog || []),
      {
        ...
        amount: damageAmount,
        message: `Dealt ${damageAmount} damage to ${enemyBattlefield[i].card.name}`
      }
    ];
  }
...
export function healTarget(
  ...
  if (targetId === 'hero') {
    ...
    updatedState.gameLog = [
      ...(updatedState.gameLog || []),
      {
        ...
        amount: healAmount,
        message: `Healed ${playerId}'s hero for ${healAmount}`
      }
    ];
  ...
  if (minionIndex !== -1) {
    ...
    if (minion.currentHealth !== undefined) {
      const maxHealth = minion.card.health || 0;
      ...
      updatedState.gameLog = [
        ...(updatedState.gameLog || []),
        {
          ...
          targetPlayerId: playerId,
          amount: healAmount,
          message: `Healed ${minion.card.name} for ${healAmount}`
        }
      ];
    }
  }
...
export function buffMinionStats(
  ...
  if (minionIndex !== -1) {
    ...
    const currentMaxHealth = minion.card.health || 0;
    ...
    updatedState.gameLog = [
      ...(updatedState.gameLog || []),
      {
        ...
        targetId: minionId,
        targetPlayerId: playerId,
        message: `Buffed ${minion.card.name} with +${attackBuff}/+${healthBuff}`
      }
    ];
  }
