...
class CardDatabaseService {
  ...
  public initialize(cards: CardData[]): void {
    ...
    const validateCards = async () => {
      try {
        // Dynamically import the validators
        const { safeCardArray, isValidCard } = await import('../utils/cardDataValidator');
        
        // Validate all cards at database initialization time
        console.log(`Validating ${cards.length} cards during database initialization...`);
        
        // Apply strict validation to all cards
        const validatedCards = safeCardArray(cards, true);
        
        console.log(`Validation complete: ${cards.length} raw cards → ${validatedCards.length} valid cards`);
        
        // Store only the validated cards
        ...
        // Fallback to non-validated initialization if validation fails
        this.cards = [...cards];
        this.buildLookupIndices();
        console.log(`Card database initialized with ${this.cards.length} cards (validation skipped).`);
        this.initialized = true;
      }
    };
  ...
  public getCardByName(name: string): CardData | undefined {
    ...
    for (const [cardName, card] of this.nameToCardMap.entries()) {
