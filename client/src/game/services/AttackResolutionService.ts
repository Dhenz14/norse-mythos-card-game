...
import { updateEnrageEffects } from '../utils/enrageUtils';
import { processFrenzyEffects } from '../utils/frenzyUtils';
import { processAfterAttackEffects } from '../utils/afterAttackUtils';
...
export function applyDamageToState(
  ...
  if (step.targetType === 'hero') {
    ...
    if (remainingDamage > 0) {
      ...
      targetPlayer.heroHealth = Math.max(0, (targetPlayer.heroHealth ?? targetPlayer.health) - remainingDamage);
    }
    ...
    if (targetPlayer.heroHealth <= 0 || targetPlayer.health <= 0) {
