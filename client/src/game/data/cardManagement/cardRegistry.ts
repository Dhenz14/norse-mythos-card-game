...
const cardRegistry: Map<number, CardData> = new Map();
...
const cardNameLookup: Map<string, number> = new Map();
...
export function registerCard(card: CardData, categories: string[] = []): void {
  ...
  // Add race/tribe if present
  if (card.race) builtInCategories.push(card.race);
  if (card.tribe) builtInCategories.push(card.tribe);
