import { CardData } from '../../../../../../types';

export const bifrostPets: CardData[] = [
	// ── Stage 1 (Basic) ──────────────────────────────────────────────
	{
		id: 50150,
		name: "Heimdall's Finch",
		manaCost: 1,
		attack: 1,
		health: 1,
		type: 'minion',
		rarity: 'common',
		description: 'Evolves after a friendly minion is buffed.',
		flavorText: 'Its song carries across all Nine Realms.',
		keywords: [],
		class: 'Neutral',
		set: 'core',
		collectible: true,
		element: 'light',
		weakness: { element: 'dark', bonusDamage: 2 },
		petStage: 'basic',
		petFamily: 'bifrost',
		petFamilyTier: 1,
		evolvesInto: 50153,
		evolutionCondition: {
			trigger: 'on_buff',
			description: 'After a friendly minion is buffed',
		},
	} as CardData,
	{
		id: 50151,
		name: 'Sun Foal',
		manaCost: 2,
		attack: 2,
		health: 2,
		type: 'minion',
		rarity: 'common',
		description: 'Evolves after dealing damage.',
		flavorText: 'Sired by the horses that pull the sun across the sky.',
		keywords: [],
		class: 'Neutral',
		set: 'core',
		collectible: true,
		element: 'electric',
		weakness: { element: 'grass', bonusDamage: 2 },
		petStage: 'basic',
		petFamily: 'bifrost',
		petFamilyTier: 1,
		evolvesInto: 50154,
		evolutionCondition: {
			trigger: 'on_deal_damage',
			description: 'After dealing damage to an enemy',
		},
	} as CardData,
	{
		id: 50152,
		name: "Sleipnir's Foal",
		manaCost: 2,
		attack: 1,
		health: 3,
		type: 'minion',
		rarity: 'common',
		description: 'Evolves after surviving a turn.',
		flavorText: 'Eight-legged even as a foal, it runs between worlds.',
		keywords: [],
		class: 'Neutral',
		set: 'core',
		collectible: true,
		element: 'neutral',
		petStage: 'basic',
		petFamily: 'bifrost',
		petFamilyTier: 1,
		evolvesInto: 50155,
		evolutionCondition: {
			trigger: 'on_survive_turn',
			description: 'After surviving a turn',
		},
	} as CardData,

	// ── Stage 2 (Adept) ──────────────────────────────────────────────
	{
		id: 50153,
		name: 'Bifrost Sentinel',
		manaCost: 0,
		attack: 2,
		health: 5,
		type: 'minion',
		rarity: 'rare',
		description: 'Divine Shield. Battlecry: Give a friendly minion Divine Shield.',
		flavorText: 'It guards the rainbow bridge with unwavering vigilance.',
		keywords: ['divine_shield', 'battlecry'],
		class: 'Neutral',
		set: 'core',
		collectible: true,
		element: 'light',
		weakness: { element: 'dark', bonusDamage: 2 },
		petStage: 'adept',
		petFamily: 'bifrost',
		petFamilyTier: 2,
		evolvesFrom: 50150,
		evolvesInto: 50156,
		evolutionCondition: {
			trigger: 'on_buff',
			description: 'After a friendly minion is buffed',
		},
		battlecry: { type: 'grant_divine_shield', targetType: 'friendly_minion', value: 1 },
	} as CardData,
	{
		id: 50154,
		name: "Arvakr's Glow",
		manaCost: 0,
		attack: 4,
		health: 3,
		type: 'minion',
		rarity: 'rare',
		description: 'Rush. Windfury.',
		flavorText: 'Arvakr blazes across the sky, pulling the sun in its wake.',
		keywords: ['rush', 'windfury'],
		class: 'Neutral',
		set: 'core',
		collectible: true,
		element: 'electric',
		weakness: { element: 'grass', bonusDamage: 2 },
		petStage: 'adept',
		petFamily: 'bifrost',
		petFamilyTier: 2,
		evolvesFrom: 50151,
		evolvesInto: 50156,
		evolutionCondition: {
			trigger: 'on_deal_damage',
			description: 'After dealing damage to an enemy',
		},
	} as CardData,
	{
		id: 50155,
		name: "Sleipnir's Shadow",
		manaCost: 0,
		attack: 3,
		health: 4,
		type: 'minion',
		rarity: 'rare',
		description: 'Taunt. Battlecry: Draw a card.',
		flavorText: "Odin's steed casts a shadow that spans all realms.",
		keywords: ['taunt', 'battlecry'],
		class: 'Neutral',
		set: 'core',
		collectible: true,
		element: 'neutral',
		petStage: 'adept',
		petFamily: 'bifrost',
		petFamilyTier: 2,
		evolvesFrom: 50152,
		evolvesInto: 50156,
		evolutionCondition: {
			trigger: 'on_survive_turn',
			description: 'After surviving a turn',
		},
		battlecry: { type: 'draw', value: 1 },
	} as CardData,

	// ── Stage 3 (Master) ─────────────────────────────────────────────
	{
		id: 50156,
		name: "Heimdall's Champion",
		manaCost: 0,
		attack: 5,
		health: 7,
		type: 'minion',
		rarity: 'mythic',
		description: 'The final form depends on its evolution path.',
		flavorText: 'The all-seeing guardian calls forth the final battle.',
		keywords: ['divine_shield', 'taunt', 'battlecry'],
		class: 'Neutral',
		set: 'core',
		collectible: true,
		element: 'light',
		petStage: 'master',
		petFamily: 'bifrost',
		petFamilyTier: 3,
		evolvesFrom: 50153,
		stage3Variants: [
			{
				fromStage2Id: 50153,
				attack: 5,
				health: 7,
				manaCost: 0,
				keywords: ['divine_shield', 'taunt', 'battlecry'],
				description: 'Divine Shield. Taunt. Battlecry: Give all friendly minions +1/+1 and Divine Shield.',
				battlecry: { type: 'buff', targetType: 'all_friendly_minions', value: 1, buffAttack: 1, buffHealth: 1, grantKeywords: ['divine_shield'] },
			},
			{
				fromStage2Id: 50154,
				attack: 7,
				health: 5,
				manaCost: 0,
				keywords: ['windfury', 'rush'],
				description: 'Windfury. Rush. Blazes across the battlefield.',
			},
			{
				fromStage2Id: 50155,
				attack: 5,
				health: 7,
				manaCost: 0,
				keywords: ['taunt', 'battlecry'],
				description: 'Taunt. Battlecry: Draw 2 cards. They cost (2) less.',
				battlecry: { type: 'draw', value: 2, manaReduction: 2 },
			},
		],
	} as CardData,
];
