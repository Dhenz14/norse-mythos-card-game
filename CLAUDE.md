...
### Completed (Yggdrasil Cosmic Campaign Map)

Pixi.js GPU-rendered "Yggdrasil from space" campaign overhaul.

- **CosmicCanvas component** (`client/src/game/components/campaign/CosmicCanvas.tsx`): 7-layer GPU-batched cosmic backdrop
  - 3 parallax star layers: deep (1500), mid (400), near (60) — twinkle phase + drift
  - Realm-colored nebula clouds with additive blending
  - 150 cosmic dust particles drifting with sine sway
  - Bezier-curved Bifrost flow particles travel between connected realms (rainbow lerp)
  - Mouse parallax shifts each layer at different rates for depth illusion
- **Campaign page restructure** (`CampaignPage.tsx`): SVG `bifrost-glow` and `node-glow` filter defs, lazy-loaded CosmicCanvas, lore text moved to bottom-left to avoid overlap
- **Realm node redesign** (`constellation-map.css`):
  - Larger 72px nodes with outer orbital ring, inner flare on hover
  - Multi-layer neon glow box-shadow + breathing animation
  - Spring-curve hover (`cubic-bezier(0.34, 1.56, 0.64, 1)`)
  - "Start Here" beacon with float animation
- **Animated Bifrost lines**: SVG `stroke-dashoffset` flow animation + Gaussian blur glow filter
- **Yggdrasil trunk**: subtle vertical golden gradient through center
- **Node positions spread** in `nineRealms.ts` to eliminate text overlap (compressed vertical range, wider horizontal spread)
- **Pet evolution playability fix** (`HandFan.tsx`, `Hand.tsx`): Stage 2/3 pets now correctly check `petEvolutionMet === true` on the precursor minion before glowing as playable. Wider detection via `evolvesFrom` field catches edge cases where `petStage` is undefined.

### Completed (Combat Audit & Bug Fixes — 20 issues)

Deep audit of poker combat system fixed 7 CRITICAL, 5 HIGH, 5 MEDIUM, 3 LOW bugs.

**Critical fixes:**
- **C1 React hooks rule violation** (`RagnarokCombatArena.tsx`): Early return moved after all `useMemo` calls — prevents crash when `combatState` toggles null↔non-null
- **C2 Hero Power buffs broken** (`useRagnarokCombatController.ts`, `usePokerCombatAdapter.ts`): `setPlayerHeroBuffs` now takes `{attack, armor}` object matching slice signature. Hero Powers with `buff_hero` effect now actually apply
- **C3 Cautious Dealer wager wired** (`pokerCombatSlice.ts` BRACE path): `reduce_fold_penalty` now refunds committed HP when folding, with pot deduction
- **C4 Fenrir the Devourer wager wired**: `all_in_buff_minions` now applies +ATK to friendly minions on showdown when player went all-in
- **C5 Orphan `fold()` method removed**: Legacy method that mutated wrong state shape stubbed to no-op
- **C6 AI double-trigger race fixed** (`useRagnarokCombatController.ts`): Removed duplicate AI invocation from `handleAction` setTimeout block — `usePokerAI` hook now owns AI turns exclusively
- **C7 Hand evaluator empty-array sentinel** (`HandEvaluator.ts`): Returns `numericValue: 0` for 0-card edge case so it always loses to real hands; prevents cache poisoning

**High priority fixes:**
- **H1 Hourglass timer max** (`RagnarokCombatArena.tsx:949`): Reads `combatState.maxTurnTime || 60` instead of hardcoded 30 — display now matches actual countdown
- **H2 Turn timer reset on phase change** (`pokerCombatSlice.ts` `advancePokerPhase`): Players get a fresh clock on every betting phase instead of inheriting whatever was left
- **H7 Reckless Bettor multiplier** (`pokerCombatSlice.ts`): `double_blinds_bonus_multiplier` now adds `multiplierBonus` to showdown multiplier as the card text promises
- **H8 All-in side pot refund** (ENGAGE case): Refund clamps to `maxHealth` and prevents negative `pot`. Adds debug log
- **H10 ATTACK with 0 commitment rejected**: Player can't cheese turn-passing by clicking Attack with no HP committed

**Medium priority fixes:**
- **M1 Reactive damage numbers setting** (`RagnarokCombatArena.tsx`): `useSettingsStore` selector replaces `.getState()` so toggling `showDamageNumbers` mid-combat updates UI
- **M7/M8 Wager AoE damage and draws applied** (`pokerCombatSlice.ts:1290`): After showdown, `wagerAoeDamagePlayer/Opponent` now actually damages enemy minions and `wagerDrawPlayer/Opponent` actually draws cards from deck
- **M12 SmartAI FAITH preflop bug** (`SmartAI.ts:71`): Removed FAITH from `isFirstBettingRound` check — AI can now check on the flop instead of being forced to bet/fold

**Low priority fixes:**
- **L3 effectiveManaCost for weapons/artifacts/armor** (`gameUtils.ts:445,456,468`): Weapon/Artifact/Armor card types now spend `effectiveManaCost` so artifact discount, outcast, fateweave, and Asgard surcharge apply correctly
- **L11 Winner field set on fold resolution**: UI displays correct winner during fold celebration; cleared on next hand

**Poker button visibility fix**: `--zone-betting-bottom: 8px → 60px` so the Attack/Engage/Brace button group has clearance from the bottom edge of the 1920×1080 virtual canvas.

### Completed (IP Cleanup & Lore Audit — 35+ card fixes)

P0 audit findings — final purge of Warcraft/Hearthstone IP holdovers.

**7 Warcraft heroes renamed** (`heroes/index.ts`):
- Rastakhan the Loa King → **Volsung the Ancestor-Caller**
- Instructor Fireheart → **Hyndla, Mistress of the Elements**
- Tess Greymane → **Hervör Wolf-Daughter** (heir to Tyrfing)
- Scabbs Cutterbutter → **Ölmod the Quick-Fingered** (Svartalfheim shadow-walker)
- Captain Hooktusk → **Eric Bloodaxe** (last Norse king of York)
- Kayn Flamebrand → **Sváfnir the Flame-Exile** (cast out from Muspelheim)
- Kurtrus Ashfallen → **Hadingus the Ash-Born** (sole survivor of Hadingsey)
- Thor Stormcaller → **Thor, Lord of Thunder** (flavor rewritten to reference Mjölnir)

**24 DeathKnight Warcraft cards renamed** (`deathknight.ts`):
- Death Coil → **Draugr's Pact**
- Army of the Dead → **Host of Niflhel**
- Remorseless Winter → **Fimbulwinter's Edge**
- Blood Boil → **Hel's Fever**
- Runeforged Blade → **Helheim Rune-Cutter**
- Icebound Fortitude → **Niflheim's Carapace**
- Dark Command → **Hel's Decree**
- Death Gate → **Helgrind**
- Bone Shield → **Bone-Lacquer Ward**
- Blood Presence → **Draugr's Hunger**
- Frost Presence → **Niflheim's Breath**
- Runeblade → **Hel-Forged Blade**
- Chains of Ice → **Hrímgrip**
- Death Knight Initiate → **Helthegn Initiate**
- Frost Wyrm → **Hrimwyrm of Jotunheim**
- Val'kyr Shadowguard → **Hel-Valkyr Shadowguard**
- Obliterate → **Soul-Severance**
- Frost Lich → **Hrim-Volva**
- Death Grip → **Hel's Reach**
- Death Gate (dup) → **Gate of Gjöll**
- All flavor text rewritten to reference Eddic sources (Hel, Niflheim, Gjöll, draugar, Helgrind, Helthegn)

**Hearthstone joke flavor purged** (`warrior.ts`, `mage.ts`, `deathknight.ts`):
- "See me after class" → Hel's overseers test wounds
- "BZZZZT" → Svartalfheim dwarven forges
- "He used to work as an accountant" → blood frenzy
- "first you face the slam" → holmgang ring
- "first rule of Brawl Club" → einherjar in Valhalla
- "block, buckle, dodge" → skald in shieldwall
- "Like a good neighbor" → host of Niflhel
- "Boiling blood is a staple" → plague of Midgard
- "Pretty self-explanatory" → bone-lattice of fallen draugar
- "run-of-the-mill skeleton" → bones along the road to Hel
- "Baaaaaaa" Sheep → **Heidrun's Charge** (Valhalla's nameless flock)
- "Chilly Peace Axe" → coal of Surtr's breath

**Echo of the Sky Sovereign** (20002): Renamed to **Echo of the Storm-Lord**, flavor "King of Olympus" replaced with Mjölnir reference (was a renamed Zeus card living in Norse neutrals).

**Race taxonomy: Norse Titan → Jotunn** (8 cards across `primordialExpansionCards.ts`, `baseCards.ts`, `neutrals/index.ts`):
- Bestla, Aesir Ascendant, Bound Underfiend, Thrymr the Imprisoned, Dread Infernal of Surtr, Voidcaller of Helheim, Titan Lord of Helheim, Titan Seedling, Titan Forge-Guard
- Cards renamed: Titan Lord → **Jotunn Lord of Helheim**, Voidcaller → **Helcaller of Niflheim**, Titan Seedling → **Jotunn Seedling**, Titan Forge-Guard → **Jotunn Forge-Guard**
- Greek `Titan` race tag preserved (Cronus, Atlas, Typhon, Porphyrion in `greekMythicMinions.ts`)
