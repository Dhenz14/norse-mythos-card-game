/**
 * Script to fix missing commas between closing brackets and collectible property in neutralSpellsAndTech.ts
 */
const fs = require('fs');
...
async function fixFile() {
  try {
    ...
    // Fix missing commas between closing brackets and collectible property
    let fixedContent = content.replace(/}(\s*)collectible:/g, '},\n      collectible:');
    
    // Write the fixed content back
    await fs.promises.writeFile(filePath, fixedContent, 'utf8');
    
    log('Fixed missing commas between closing brackets and collectible property in neutralSpellsAndTech.ts');
    return { success: true };
  } catch (error) {
