...
## Extension Management (MANDATORY)
At the start of every new chat, you MUST automatically optimize the VS Code extensions for the current development context by running:
```
powershell -ExecutionPolicy Bypass -File ".vscode\scripts\extension-chat-helper.ps1" -Analyze
```
This ensures optimal performance and context-specific tools are always available without manual intervention. This rule is mandatory and takes precedence over any other considerations.
...
- **Server Tools**: The following MCP (Methodological Cognitive Pattern) tools are available through the MCP server for development work:
  1. **mcp_sequential_thinking**: Server tool for breaking down tasks into sequential steps and dependencies
  2. **mcp_think_tool**: Server tool for analyzing and selecting optimal development tools
  3. **mcp_code**: Server tool for implementing and validating code patterns
  4. **mcp_mermaid**: Server tool for generating and managing architectural diagrams
  5. **mcp_21st_devmagic**: Server tool for applying modern development practices

IMPORTANT: MCPs are server tools that must be invoked through proper tool calls, not guidelines or patterns to follow manually. Each MCP tool provides specific functionality and should be used through the appropriate server interface.

## Example Tool Usage

```typescript
// Example of proper MCP tool usage
interface MCPToolCall {
  tool: 'mcp_sequential_thinking' | 'mcp_think_tool' | 'mcp_code' | 'mcp_mermaid' | 'mcp_21st_devmagic';
  params: {
    task: string;
    context: object;
    options?: object;
  };
  return: Promise<MCPResult>;
}
```

## Example MCP Tool Applications

**Example: Using MCP Tools for Authentication System**
```typescript
// 1. Use Sequential Thinking tool to break down the task
const sequentialPlan = await mcp_sequential_thinking({
...
// 2. Use Think Tool to analyze implementation options
const toolAnalysis = await mcp_think_tool({
  task: "Select authentication tools",
  context: {
    options: ["bcrypt", "JWT", "Redis"],
    requirements: {
      security: "high",
...
// 3. Use Code MCP to implement patterns
const codeImplementation = await mcp_code({
  task: "Implement authentication patterns",
  context: {
...
// 4. Use Mermaid MCP for architecture diagrams
const architectureDiagrams = await mcp_mermaid({
  task: "Generate auth flow diagrams",
  context: {
    diagrams: ["flow", "data-model", "sequence"],
    output: "./docs/diagrams"
  }
});

// 5. Use 21st DevMagic for modern practices
const modernImplementation = await mcp_21st_devmagic({
  task: "Apply modern auth practices",
  context: {
    practices: ["zero-trust", "edge-computing"],
    security: "contextual-awareness"
  }
});
```

**Example: Using MCP Tools for Card Effect Development**
```typescript
// 1. Sequential breakdown of card effect implementation
const effectPlan = await mcp_sequential_thinking({
  task: "Implement Legendary Card Holographic Effect",
  context: {
    phases: ["asset-prep", "effect-impl", "perf-verify", "integration"],
    constraints: {
      texture: { maxDim: 2048, budget: "512MB" },
      shader: { frameTime: "16ms" },
      animation: { duration: "300ms" }
    }
  }
});

// 2. Tool selection for effect development
const toolSelection = await mcp_think_tool({
  task: "Select effect development tools",
  context: {
    tools: {
      performance: ["Chrome DevTools", "WebGL Inspector"],
      monitoring: ["FPS Monitor", "Memory Profiler"]
    }
  }
});

// 3. Pattern implementation for effect code
const effectImplementation = await mcp_code({
  task: "Implement effect patterns",
  context: {
    patterns: ["ShaderComposition", "StateMachine", "Observer"],
    performance: { target: "60fps" }
  }
});

// 4. Technical diagrams for effect pipeline
const effectDiagrams = await mcp_mermaid({
  task: "Generate effect pipeline diagrams",
  context: {
    diagrams: ["render-pipeline", "state-flow"],
    output: "./docs/effects"
  }
});

// 5. Modern rendering techniques
const modernTechniques = await mcp_21st_devmagic({
  task: "Apply modern rendering techniques",
  context: {
    techniques: ["instanced-rendering", "shader-permutations"],
...
When approaching any development task, systematically apply the 5 MCPs:

**Example: Applying MCPs to Feature Development**
```
Task: Implement user authentication system

1. Sequential Thinking
   - Break down implementation into ordered steps: database schema → 
     authentication logic → API endpoints → frontend integration
   - Process each component fully before advancing to dependencies

2. Think Tool
   - Analytically select optimal tools for each development phase
   - For authentication: bcrypt for password hashing, JWT for tokens,
     Redis for session management (based on systematic evaluation)

3. Code-MCP
   - Apply established patterns: Repository pattern for data access,
     Service pattern for business logic, DI for component interaction
   - Structure code following consistent organizational principles

4. Mermaid-MCP-Server
   - Create architectural diagrams before implementation:
     - Authentication flow diagram
     - Data model relationships
     - API interaction sequences
   - Use diagrams to validate approach before coding begins
   - Archive all diagrams in the project's `/docs/diagrams` directory with consistent naming:
     - `{component}-{diagram-type}-{YYYY-MM-DD}.md` (e.g., `auth-flow-2025-04-19.md`)
   - Reference diagrams in related documentation and code comments
   - Include diagram references in the BUG_MEMORY.md when changes affect architecture
   - Update existing diagrams when architectural changes are made, preserving version history

5. 21st DevMagic
   - Implement progressive enhancement techniques
   - Apply modern paradigms: Zero-trust security model, Edge computing principles
   - Incorporate contextual awareness in business logic
```

**Example: Applying MCPs to Card Effect Development**
```
Task: Implement Legendary Card Holographic Effect
...
1. Sequential Thinking
   - Break down implementation into ordered steps:
     1. Asset Preparation
        - Verify texture dimensions (2048x2048 max)
        - Check memory budget (< 512MB total)
        - Prepare fallback assets for lower settings
     
     2. Effect Implementation
        - Implement core shader logic (16ms budget)
        - Add state management hooks for animations
        - Set up animation timeline (300ms max)
     
     3. Performance Verification
        - Measure FPS impact (maintain 60 FPS)
        - Profile memory usage (per card budget)
        - Validate state transitions (50ms max)
     
     4. Integration Testing
        - Test across card states (hand/field/hover)
        - Verify network sync (100ms threshold)
        - Validate interaction timing (16ms response)

2. Think Tool
   - Analytically select optimal tools for each phase:
     - Chrome DevTools Performance tab for frame analysis
     - WebGL Inspector for shader debugging
     - Custom FPS monitoring for performance validation
     - Memory profiler for texture budget verification

3. Code-MCP
   - Apply established patterns:
     - Shader composition pattern for effect layers
     - State machine pattern for animation phases
     - Observer pattern for performance monitoring
     - Factory pattern for effect instantiation

4. Mermaid-MCP-Server
   - Create technical diagrams:
     - Effect render pipeline flow
     - State transition diagram
     - Memory allocation map
     - Performance monitoring points
   - Archive diagrams as:
     - `effects-pipeline-{YYYY-MM-DD}.md`
     - `state-flow-{YYYY-MM-DD}.md`

5. 21st DevMagic
   - Implement modern rendering techniques:
     - Instanced rendering for multiple cards
     - Shader permutations for quality settings
     - Adaptive quality based on performance
```

### Phase 1: Understand & Research
1. **Extract Core Intent**
   - Analyze the request to understand the underlying goal beyond literal instructions
   - Identify expected outcomes and success criteria
   
   **Example:**
   ```
   User request: "Add user search functionality"
   
   Core intent analysis:
   - Primary goal: Allow filtering users by search criteria
   - Implied requirements: 
     - Fast response time for large user sets
...
2. **Autonomous Context Discovery**
   - **CRITICAL DIRECTIVE**: For ANY ambiguity or missing information, DEFAULT to investigation using tools rather than asking questions
   - Map relevant code locations using:
     - `list_dir`: Explore directory structures
     - `file_search`: Locate relevant files
     - `grep_search`: Find specific patterns
     - `codebase_search`: Understand code relationships
   - Study existing implementations using `read_file` to understand:
     - Current patterns and conventions
     - Implementation logic
     - Configuration structures
   - Document all discovered context that resolves ambiguities
